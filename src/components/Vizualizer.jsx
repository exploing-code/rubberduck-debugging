...
export default function Visualizer() {
  const { activeDuck, isAudioCtxActivated, partyOn, activeSectionNumb } =
    myContext();
  ...
  useEffect(() => {
    if (duckCtx && !duckSourceNodeRef.current && duckSoundRef.current) {
      const duckSourceNode = duckCtx.createMediaElementSource(
    ...
    if (duck2Ctx && !duck2SourceNodeRef.current && duckSoundRef2.current) {
      const duck2SourceNode = duck2Ctx.createMediaElementSource(
        duckSoundRef2.current
      );
      ...
      if (analyzerNodeRef.current) {
        ...
        if (volumePercent > volumeThreshold) {
          if (!isVolumeAboveThreshold) {
            ...
            if (
              timePassed >= awaitResponseTime &&
              !animationRan &&
              !isPartyOnRef.current
            ) {
              runAnimation();
              animationRan = true;
            }
  ...
  return (
    <div className=' absolute flex w-full h-[90vh] bottom-0 items-center justify-center'>
      ...
      <div
        ref={visualizerRef}
        style={{ backgroundColor: ducks[activeDuck].thirdClr }}
        className=' absolute bottom-0 w-full transition-all duration-[0.05s]'
      ></div>
      <h1
        ref={titleRef}
        style={{ color: ducks[activeDuck].secondaryClr }}
        className=' opacity-0 text-[30vw] leading-[25vw] z-30 pointer-events-none scale-0'
      >
        QUACK
        <br />
        QUACK
